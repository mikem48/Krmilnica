...
router.get('/:id/settings', (req, res) => {
  ...
  db.get(query, [deviceId, req.user.id], (err, device) => {
    ...
    db.get('SELECT * FROM firmware ORDER BY upload_date DESC LIMIT 1', [], (fwErr, latestFirmware) => {
      ...
      const settings = {
        ...params,
visina: params.visina ?? device?.visina ?? null,
  wifi_cas: params.wifi_cas ?? device?.wifi_cas ?? null,
  obvestilo_napetost: params.obvestilo_napetost ?? device?.obvestilo_napetost ?? null,
  obvestilo_krmilo: params.obvestilo_krmilo ?? device?.obvestilo_krmilo ?? null,
  obvestilo_stevilka: params.obvestilo_stevilka ?? device?.obvestilo_stevilka ?? null,

  ura1_h: params.ura1_h ?? device?.ura1_h ?? null,
  ura1_min: params.ura1_min ?? device?.ura1_min ?? null,
  ura2_h: params.ura2_h ?? device?.ura2_h ?? null,
  ura2_min: params.ura2_min ?? device?.ura2_min ?? null,

  casovnik2: toBool(params.casovnik2 ?? device?.casovnik2),
  cas_delovanja: params.cas_delovanja ?? device?.cas_delovanja ?? null,
  hitrost_motorja: params.hitrost_motorja ?? device?.hitrost_motorja ?? null,

  pon: toBool(params.pon ?? device?.pon),
  tor: toBool(params.tor ?? device?.tor),
  sre: toBool(params.sre ?? device?.sre),
  cet: toBool(params.cet ?? device?.cet),
  pet: toBool(params.pet ?? device?.pet),
  sob: toBool(params.sob ?? device?.sob),
  ned: toBool(params.ned ?? device?.ned),
